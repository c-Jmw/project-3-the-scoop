...
function createArticle(url, request) {
  ...
  if (requestArticle && requestArticle.title && requestArticle.url &&
    ...
    const article = {
      ...
      commentIds: requestArticle.commentIds,
      upvotedBy: [],
      downvotedBy: []
    };
...
function updateComment( url, request ) {
  ...
  if ( !id || !requestComment ) {
    ...
    savedComment.upvotedBy = requestComment.upvotedBy || savedComment.upvotedBy;
    savedComment.downvotedBy = requestComment.downvotedBy || savedComment.downvotedBy;
...
function deleteComment( url, request ) {
  ...
  if (savedComment) {
    // get savedComment comment id
    const commentId = database.comments[ id ];
    // find correlating comment id in users and articles
    const userCommentIds = database.users[ commentId.username ].commentIds;
    const articleComment = database.articles[ commentId.articleId ].commentIds;
